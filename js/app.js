...
function myjsapp(peerClient) {
    ...
    return {
        ...
        createChatWindow: function(id) {
            ...
            var message = $('<input type="text" class="form-control" placeholder="Ingrese mensaje">')
            ...
            $('.panel-body', panel).append('<span class="text-primary">Usted ahora esta chateando</span>').append(history)
        ...
        showIncomingCall : function (peerId, options) {
            ...
            if(options['video'])
                var txt = "Contestar video llamada de : " + peerId
            else
                var txt = "Contestar llamada de  : " + peerId
        ...
        updateOnlieUsers : function (users) {
            ...
            list.empty()
            if(users.length == 0) {
                var usr = '<li>Aún hay usuarios activos</li>'
                list.append(usr);
                return
            }
